...
Codex's interactive TUI has no tracked session-open or instruction re-emit channel and is not covered by either tier; its separate `PreCompact` hook captures Stow evidence but does not re-inject session instructions.
...
| Codex interactive TUI | Uncovered | The separate `PreCompact` hook is Stow-only. | Codex 0.146.0 does not fire the tracked project `SessionStart` hook in its interactive TUI; Firstmate ships no global instruction-refresh hook or re-emit channel and does not claim instruction-refresh delivery for this surface. |
...
Cursor's instruction-refresh compaction surface is uncovered in the same sense as Codex's interactive TUI above: Firstmate registers no Cursor-owned `preCompact` adapter, and the Claude-compatible Stow entry stands down on Cursor payloads, so a compacted Cursor session keeps whatever context survived rather than receiving a fresh digest.
