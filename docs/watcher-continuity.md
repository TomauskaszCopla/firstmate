...
While one routine main wake is still unconsumed, later routine signal/stale closes reuse its already-queued drain prompt instead of adding more Pi follow-ups; their durable rows and replacement handoff remain intact, and check-kind closes and continuity failures still get their own prompt.
After Pi consumes the prompt, another close may send a new one.
