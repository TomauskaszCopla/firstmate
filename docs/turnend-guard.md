...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  ...
  The guarded set is the `SessionStart` entry, the `PreCompact` entry, the two `PreToolUse` Bash entries, and both `Stop` entries.
...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries both `Stop` entries, the `SessionStart` entry, the `PreCompact` entry, and the two `PreToolUse` Bash entries; Grok owns its Stop, SessionStart, and PreToolUse routes, while automatic pre-compaction Stow is supported only for Codex and Claude.
